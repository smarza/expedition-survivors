...
- opening/reserializing the project in Unity `6000.5.4f1` on the development machine;
- rebuilding and smoke-testing Windows executables with the patched runtime;
- shared simulation implementation;
- Unity Test Framework suites;
...
| `GameDirector` | Local run state, spawning, progression and orchestration. |
...
| `OnlineCoopSpike` | Current monolithic host/client transport, online simulation and online UI. |
...
`GameDirector` and `OnlineCoopSpike` contain overlapping combat and progression rules. Adding every future weapon, status, evolution or character twice would create inconsistent balance, network bugs and unsustainable QA. Resolving this duplication is the main purpose of 0.8.0.
...
- no formal Unity EditMode/PlayMode suite yet;
- no production cloud-save integration;
- save migrations need expansion before public progression begins;
...
Exit gate: Solo runs use the shared model with no feature loss.
...
2. Ask the user to clone/pull and open the branch with Unity `6000.5.4f1`.
3. Capture and commit any legitimate Unity reserialization/package-lock changes.
4. Confirm Solo, Local and Online still work on the patched Editor and newly rebuilt executables.
5. Introduce Unity test assemblies and the first deterministic rule tests.
6. Propose the smallest safe extraction of the shared run model.
7. Keep the game playable after each extraction step.
